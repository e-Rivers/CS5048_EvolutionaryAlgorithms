...
for func_name in functions:
    ...
    ga_df = pd.read_csv(f'/home/e-rivers/Documents/MASTERs/3rdSemester/EvolutionaryAlgorithms/CS5048_EvolutionaryAlgorithms/HMWK_3/results_ga_constrained/{func_name}_results.csv')
    de_df = pd.read_csv(f'/home/e-rivers/Documents/MASTERs/3rdSemester/EvolutionaryAlgorithms/CS5048_EvolutionaryAlgorithms/HMWK_3/results_de_constrained/{func_name}_results.csv')

    # Extract the best fitness for each execution (aqui sustituimos por lo que vayamos a usar fitness o penalty)
    ga_best = ga_df.groupby('Execution')['Penalty'].min()
    de_best = de_df.groupby('Execution')['Penalty'].min()
